...
class Customer(Base):
    ...
    wechat_status: Mapped[str | None] = mapped_column(String(20), nullable=True)
    """微信状态，如：未添加、已加上微信"""
    ...
    primary_project: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """一级项目"""

    project: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """项目"""

    business_dept: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """事业部"""

    call_dept: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """呼叫部"""

    call_group: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """呼叫组"""

    advertiser: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """广告商"""

    landing_page: Mapped[str | None] = mapped_column(String(256), nullable=True)
    """着陆页"""

    # ==================== 分配信息 ====================
    assign_method: Mapped[str | None] = mapped_column(String(20), nullable=True)
    """分配方式，如：手动、自动"""

    assign_type: Mapped[str | None] = mapped_column(String(32), nullable=True)
    """分配类型，如：公海领取、主管调配"""
    ...
    creator_org: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """创建人归属机构"""

    # ==================== 咨询信息 ====================
    first_consultant: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """首次咨询师"""

    last_consultant: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """最后咨询师"""

    first_assign_org: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """首次分配归属机构"""

    first_assign_person: Mapped[str | None] = mapped_column(String(64), nullable=True)
    """首次分配归属人"""
    ...
    ip_city: Mapped[str | None] = mapped_column(String(32), nullable=True)
    """IP城市"""
    ...
    # ==================== 系统字段 ====================
    original_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
